...
const CombinedDashboard: React.FC = () => {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setLiveMetrics(prev => ({
        totalBuildings: prev.totalBuildings + Math.floor(Math.random() * 50),
        processingSpeed: prev.processingSpeed + (Math.random() - 0.5) * 10,
        ...
        queueLength: Math.max(0, prev.queueLength + Math.floor((Math.random() - 0.5) * 2))
      }));
    }, 2000);
    ...
    <div className="min-h-screen bg-gray-900 text-white">
      ...
      <main className="max-w-7xl mx-auto px-6 py-8">
        ...
        <div className="flex gap-1 mb-8 bg-gray-800 rounded-lg p-1">
          {['overview', 'analytics', 'pipeline', 'maps', 'live'].map((tab) => (
            <button
              key={tab}
